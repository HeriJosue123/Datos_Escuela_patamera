...
function runPremiumTransition(config, isExternal, onMidPoint, onEnd) {
    ...
    // Punto medio (cambiar interfaz silenciosamente en segundo plano)
    setTimeout(() => {
        if(onMidPoint) onMidPoint();
    }, 800); // 800ms
    
    // Fin (desaparecer o redirigir)
    setTimeout(() => {
        stopLoadingMessages();
        
        if (isExternal) {
            document.getElementById('global-transition-subtitle').innerText = "Abriendo módulo...";
            if(onEnd) onEnd();
        } else {
    ...
    }, 1500); // 1.5 segundos de duración total para poder apreciar el diseño
}
...
window.navigateWithTransition = function(href, tabId) {
    ...
    runPremiumTransition(config, true, null, () => {
...
window.triggerInternalTransition = function(tabId, callback) {
    ...
    runPremiumTransition(config, false, 
        () => {
